            ...
            <div class="bg-white p-6 rounded-lg shadow-md">
                <table class="w-full table-auto border-collapse">
                    ...
                    @foreach($cartProducts as $product)
                        <tr class="border-b">
                            <td class="px-4 py-2 font-semibold text-gray-700">{{ $product->name }} <span class="text-xs text-gray-500 ml-2">Dostupno: {{ $product->amount }}</span></td>
                            ...
                            <td class="px-4 py-2">
                                <form method="POST" action="{{ route('cart.update') }}">
                                    ...
                                    <input type="number" name="quantity" value="{{ $product->quantity }}" min="1" max="{{ $product->amount }}"
