    ...
    @if(Auth::check() && Auth::user()->role == "admin")
        <section class="bg-gray-500 p-5 my-5 rounded">
            ...
            <ul class="flex my-3 gap-3 text-white">
                <a class=" rounded bg-gray-700 text-white p-2 hover:bg-gray-200 hover:text-black transition:ease-in-out duration-700"
                   href="{{route("admin.addproduct")}}">Dodaj proizvod</a>
                <a class=" rounded bg-gray-700 text-white p-2 hover:bg-gray-200 hover:text-black transition:ease-in-out duration-700"
                   href="{{route("admin.products")}}">Svi proizvodi</a>
            </ul>
        </section>
    @endif
